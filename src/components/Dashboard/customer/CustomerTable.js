...
const headCells = [
  ...
  { id: 'mobile', numeric: true, disablePadding: true, label: 'Phone Number' },
  { id: 'email', numeric: true, disablePadding: true, label: 'EmailId' },
  { id: 'username', numeric: true, disablePadding: true, label: 'Status' },
  { id: 'order', numeric: true, disablePadding: false, label: 'Orders' },
...
export default function EnhancedTable(props) {
  ...
  const [rows, setCustomers] = React.useState([])
  ...
  React.useEffect(() => {
    async function orders(){
      try{
        const prds = await myApi.get('/api/store_detail/customers/')
        setCustomers(prds.data)
        console.log(rows)
      }catch(err){
        alert(err)
      }
  ...
  const deleteOrder = (order_id) =>{
    ...
    // const condi =  handleClickOpen()
    // console.log(condi,"condi")
    // if(condi){
    const data = myApi.delete("/api/store_detail/customers/",{data:{id:order_id}})
    setCustomers((item)=>{
  ...
  return (
    ...
    <div className={classes.root}>
      ...
      <RouterLink to={`/dashboard/customer/edit/${row.id}`}>
